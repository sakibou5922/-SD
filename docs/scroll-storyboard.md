...
> **v1.3 補記（2026-09-24, SD-cosmic-bright）**: モーフ列を **環（Hero: 惑星メッシュ + 環の点群）→ 原子軌道（About）→ 格子 → 設計図 → 結び目 → 礎 → 輪** に変更。ピン留めは全廃（Service は 2 カラム、Process はジグザグ）。3D の配置は `sections.ts` の `P`（セクションごとの group 位置・スケール）で管理し、前セクションの末尾 0.75–1.0 で次の位置へ移動する。詳細は `docs/cosmic-bright-3d.md`。
